...
def make_sparse_coded_signal(n,N,k,Ndata, dictionary="randn", use_sklearn=True, random_state=None):
    """
    ...
    if dictionary == "randn" and use_sklearn and has_sklearn_datasets:
        ...
        for i in range(Ndata):
            support[:, i] = rng.permutation(N)[:k]
            gamma[support[:, i],i] = rng.randn(k)
...
def make_compressed_sensing_problem(m, n,N,k,Ndata, dictionary="randn", acquisition="randn", use_sklearn=True, random_state=None):
    """
    ...
    data, dictionary, gamma, support = make_sparse_coded_signal(n,N,k,Ndata, dictionary, use_sklearn, random_state=rng)

    # generate acquisition matrix
    if acquisition=="randn":
        acqumatrix = rng.randn(m, n)
...
def make_cosparse_coded_signal(n, N, l, numdata, operatortype="tightframe", random_state=None):
    """
    ...
    for i in range(numdata):
        cosupport[:,i] = numpy.sort(rng.permutation(N)[:l])
        [U,D,Vh] = numpy.linalg.svd(operator[cosupport[:,i],:])
        V = Vh.T
        nullspace = V[:,l:]
        data[:,i] = numpy.squeeze(numpy.dot(nullspace, rng.randn(n-l,1)))
...
def make_analysis_compressed_sensing_problem(m, n, N, l, numdata, operator="tightframe", acquisition="randn", random_state=None):
    """
    ...
    data, operator, gamma, cosupport = make_cosparse_coded_signal(n, N, l, numdata, operator, random_state=rng)
